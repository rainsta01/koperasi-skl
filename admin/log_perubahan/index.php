...
$stmt = $conn->prepare("SELECT log_perubahan.*, users.nama FROM log_perubahan JOIN users ON log_perubahan.user_id = users.id ORDER BY waktu DESC LIMIT 100");
$stmt->execute();
...
<body>
    ...
    <h2>Riwayat Perubahan Database</h2>
    <table border="1">
        <tr>
            <th>No</th>
            <th>User</th>
        ...
        <tr>
            ...
            <td>
                <?php echo htmlspecialchars($row['tabel']); ?>
            </td>
            <td>
                <pre><?php echo htmlspecialchars($row['data_lama']) ?: '-'; ?></pre>
            </td>
            <td>
                <pre><?php echo htmlspecialchars($row['data_baru']) ?: '-'; ?></pre>
            </td>
            <td>
